...
1. main에서만 작업한다.
2. 작업 후 git add / commit / main으로 push 까지 진행한다.
3. node version이 18.0.0이상 인지 확인하고
4. npm run deploy를 진행하면 알아서 publish된다.

## gatsby를 못 찾아서 gatsby develop이 안될 경우

```
sudo npm install -g gatsby-cli : global로 다운받기

gatsby -v : 버전 확인하여 잘 다운받아졌는지 확인!
```
